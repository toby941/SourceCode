...
public class MapABCUtil {
	...
	private final static String XML_URL = "<?xml version=\"1.0\" encoding=\"gb2312\"?>\r\n"
	...
	public static String executeHttpGet(String requestUrl)
		...
		if (HTTP_RESPONSE_STATUS_SUCCESS_CODE
			...
			logger.error("execute map abc erro  r: "
					+ ReflectionToStringBuilder.toString(statusLine));
		}
		return StringUtils.EMPTY;
	}

	public static boolean isValidLongitudeAndLatitude(String longitude,
			String latitude) {
		return StringUtils.isNotBlank(longitude)
				&& StringUtils.isNotBlank(latitude) && !"0".equals(longitude)
				&& !"0".equals(latitude);
	}

	public static LocationInfo getLocationInfo(String longitude, String latitude)
			throws ParseException, IOException {
		if (isValidLongitudeAndLatitude(longitude, latitude)) {
			String xmlParameter = MessageFormat.format(XML_URL, longitude,
					latitude);
	...
	public static String getMapABCRequestURL(String xml, String apiKey)
			throws UnsupportedEncodingException {
		return MessageFormat.format(MAPABC_URL,
				URLEncoder.encode(xml, "utf-8"), apiKey);
	}
