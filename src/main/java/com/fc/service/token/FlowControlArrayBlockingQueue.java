 ...
 * @since 2013年7月29日
 */
public class FlowControlArrayBlockingQueue<E> extends AbstractQueue<E> implements BlockingQueue<E>,
		java.io.Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = -8338248196559087704L;
	/** The queued items */
	...
	private TokenDispatch dispatch;

	private Integer priority;

	private Float threshold = Constants.DEFAULT_TOKEN_BORROW_THRESHOLD;
	...
	public TokenDispatch getDispatch() {
		return dispatch;
	}

	public void setDispatch(TokenDispatch dispatch) {
	...
	/**
	 * Creates an <tt>LimitArrayBlockingQueue</tt> with the given (fixed)
	 ...
	 */
	public FlowControlArrayBlockingQueue(int capacity) {
		this(capacity, false);
	}

	public FlowControlArrayBlockingQueue(int capacity, boolean fair, Float threshold) {
		this(capacity, fair);
	 ...
	 *            insertion or removal, are processed in FIFO order; if <tt>false</tt> the access order is unspecified.
	 * @throws IllegalArgumentException
	 *             if <tt>capacity</tt> is less than 1
	 */
	public FlowControlArrayBlockingQueue(int capacity, boolean fair) {
		if (capacity <= 0)
	...
	public boolean isCanborrow() {
		...
		if (capacity == 0) {
			...
			return (float) size / (float) capacity > threshold;
		}
	}
	 ...
	 */
	public FlowControlArrayBlockingQueue(int capacity, boolean fair, Collection<? extends E> c) {
		this(capacity, fair);
		if (capacity < c.size())
			throw new IllegalArgumentException();
	 ...
	 * to do so immediately without exceeding the queue's capacity, returning <tt>true</tt> upon success and throwing an
	...
	/**
	 * Inserts the specified element at the tail of this queue if it is possible
	 * to do so immediately without exceeding the queue's capacity, returning <tt>true</tt> upon success and
	 * <tt>false</tt> if this queue is full. This
	 ...
	 * @throws InterruptedException
	 */
	public E pollNotDispatch(long timeout, TimeUnit unit) throws InterruptedException {
		long nanos = unit.toNanos(timeout);
		final ReentrantLock lock = this.lock;
		lock.lockInterruptibly();
		try {
	 ...
	 * is present. More formally, removes an element <tt>e</tt> such that <tt>o.equals(e)</tt>, if this queue contains
	...
	/**
	 * Iterator for LimitArrayBlockingQueue
	 */
	private class Itr implements Iterator<E> {
		...
		@Override
		public E next() {
			final ReentrantLock lock = FlowControlArrayBlockingQueue.this.lock;
			lock.lock();
			try {
				if (nextIndex < 0)
