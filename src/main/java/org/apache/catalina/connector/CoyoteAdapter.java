...
import com.fc.container.TwiceReadRequest;
...
public class CoyoteAdapter implements Adapter {
	...
	private static final String POWERED_BY = "Servlet/2.5 JSP/2.1 " + "(" + ServerInfo.getServerInfo() + " Java/"
	...
	protected static final boolean ALLOW_BACKSLASH = Boolean.valueOf(
			System.getProperty("org.apache.catalina.connector.CoyoteAdapter.ALLOW_BACKSLASH", "false")).booleanValue();
	...
	public void service(org.apache.coyote.Request req, org.apache.coyote.Response res) throws Exception {
		...
		if (request == null) {
			...
			// 若request已经存在，则每次获取后进行body的reset
			TwiceReadRequest twiceReadRequest = (TwiceReadRequest) request;
			twiceReadRequest.resetBody();
		}
		...
		try {
			...
			if (postParseRequest(req, request, res, response)) {
				...
				if (com.fc.util.Constants.METHOD_GET.equals(request.getMethod())) {
					allow = TokenPoolManager.getTokenPoolManager().getToken(uri, request.getQueryString());
				} else {
					allow = TokenPoolManager.getTokenPoolManager().getToken(uri, request.getInputStream());
				}
				if (!allow) {
					if (log.isDebugEnabled()) {
						log.debug(String.format("get token fail with uri:%s", uri));
					}
					response.sendError(HttpServletResponse.SC_SERVICE_UNAVAILABLE,
							MessageFormat.format("can not obtain a token so end request,with requestURI:{0}", uri));
				} else {
					if (log.isDebugEnabled()) {
						log.debug(String.format("get token success with uri:%s", uri));
					}
					connector.getContainer().getPipeline().getFirst().invoke(request, response);
				}
				// 请求执行完毕后释放token
				TokenPoolManager.getTokenPoolManager().releaseToken(uri);
				if (request.isComet()) {
	 ...
	 * Normalize URI.
	 * <p>
	 * This method normalizes "\", "//", "/./" and "/../". This method will return false when trying to go above the
	 * root, or if the URI contains a null byte.
	 * @param uriMB
	 ...
	 * <p>
	 * This method checks for "\", 0, "//", "/./" and "/../". This method will return false if sequences that are
	 * supposed to be normalized are still present in the URI.
	 * @param uriMB
